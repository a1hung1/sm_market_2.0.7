...
class Data extends AbstractHelper
{
	...
	{
		if (!(int)$this->getEnableDisable()) return;
		if (!define('MAGENTECH2_JQUERY', 1) && (int)$this->getEnableDisable())
		{
			$_jquery_libary = 'Sm_ImageSlider::js/jquery-2.1.4.min.js';
	...
	{
		if (!(int)$this->getEnableDisable()) return;
		if (!define('MAGENTECH2_JQUERY_NOCONFLICT', 1) && (int)$this->getEnableDisable())
		{
			$_jquery_libary = 'Sm_ImageSlider::js/jquery-noconflict.js';
			return $_jquery_libary;
		}
	}

	public function includeJqueryOwlCarosel()
	{
		if (!(int)$this->getEnableDisable()) return;
		$_jquery_libary = 'Sm_ImageSlider::js/owl.carousel.js';
		return $_jquery_libary;
	}
	...
	public function _resizeImage($image, $config = [], $type = "product", $folder = 'resized')
	{
		...
		if (file_exists($cache_dir . '/' . $_file_name) && @getimagesize($cache_dir . '/' . $_file_name) !== false) {
			$new_image = $this->_getMegaMenuDirMedia().'catalog/' . $type . '/' . $folder . '/' . $config['width'] . 'x' . $config['height'] . '/' . md5(serialize($config)) . '/' . $_file_name;
