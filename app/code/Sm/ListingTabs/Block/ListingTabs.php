...
class ListingTabs extends \Magento\Catalog\Block\Product\AbstractProduct
{
	...
	)
	{
		...
		if ($this->getRequest() && $this->getRequest()->isAjax()) {
			$_cfg =  $this->getRequest()->getParam('config');
	...
	public function _getCfg($attr = null , $data = null)
	{
		...
		if(isset($_GET['dev']) && $_GET['dev'] == 'duydh'){
			//var_dump($this->_scopeConfigInterface->getValue('listingtabs', \Magento\Store\Model\ScopeInterface::SCOPE_STORE , '1' ),  \Magento\Store\Model\ScopeInterface::SCOPE_STORE,$this->_storeId);die('here');
		}
	...
	protected function _toHtml()
	{
		...
		if ($this->_isAjax()) {
			...
			if ($use_cache){
				$cacheid_items = md5(serialize(array ($this->_getConfig(), $ajax_listingtags_start, $catid)));
				if ( $dataitems = $Cache_Lite->get($cacheid_items)) {
					return  $dataitems;
				} else { 
					$template_file = "sm/listingtabs/default_items.phtml";
			...
			if ($use_cache){
				...
				if ($data = $Cache_Lite->get($hash)) {
					...
					$template_file = (!empty($template_file)) ? $template_file : "Sm_ListingTabs::sm/listingtabs/default.phtml";
					$this->setTemplate($template_file);
					$data = parent::_toHtml();
			...
			case 'categories':
				...
				foreach ($cats as $cat) {
					...
					if (isset($cat->count) && $cat->count) {
						$cat->id = $cat->entity_id;
						if ($cat->entity_id== $catidpreload) {
							$cat->sel = 'sel';
							$cat->child = self::_getProductInfor($catidpreload);
							$selected = true;
						}
						$list[$cat->id] = $cat;
					}
	...
	public function _getProductInfor($_catids, $field_order = null)
	{
		...
		if (!empty($_catids)) {
			$products = self::_getProductsBasic($_catids, $field_order);
			if ($products != null) {
	...
	public function _getCatinfor($catids, $orderby = null)
	{
		...
		if (!empty($catids)) {
			foreach ($catids as $catid) {
				...
				$cat->count = self::_countProducts($catid);
	...
	*/
	...
	{
		!is_array($catids) && settype($catids, 'array');
		$countProduct = self::_getProductsBasic($catids, $field_order, true);
		return $countProduct;
	}
