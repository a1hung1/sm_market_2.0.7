...
class Categories extends \Magento\Catalog\Block\Product\AbstractProduct
{
	...
	public function _getCfg($attr = null , $data = null)
	{
		...
		$defaults = [];
		$collection = $this->_scopeConfigInterface->getValue('categories', \Magento\Store\Model\ScopeInterface::SCOPE_STORES,$this->_storeId);
		...
		$_configs = $this->_scopeConfigInterface->getValue('categories', \Magento\Store\Model\ScopeInterface::SCOPE_STORES,$this->_storeId);
		if (empty($_configs)) return;
		$cfgs = [];

		foreach ($groups as $group) {
			$_cfgs = $this->_scopeConfigInterface->getValue('categories/'.$group.'', \Magento\Store\Model\ScopeInterface::SCOPE_STORES,$this->_storeId);
	...
	public function _getCatinfor($catids)
	{
		...
		if (!empty($catids)) {
			...
			foreach ($catids as $catid) {
				...
				$list[$_cat['id']] = (object)$_cat->getData();
			}
		}
		return $list;
	}
	...
	public function _getProductsBasic($catids, $countProduct = false)
	{
		...
		!is_array($catids) && settype($catids, 'array');
		if (!empty($catids)) {
			$attributes = $this->_objectManager->create('Magento\Catalog\Model\Config')->getProductAttributes();
